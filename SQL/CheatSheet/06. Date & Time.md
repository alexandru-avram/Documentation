...
# 06. Date & Time — PostgreSQL • Oracle • T‑SQL

> Cross‑dialect cheatsheet using a consistent pattern:
>
> ### FUNCTION / CONCEPT
> Short description.
>
> ```sql
> -- Code examples (annotated with (PG), (Oracle), (T-SQL) where relevant)
> ```
...
## Extracting parts (year/month/day, names)
...
### Common T-SQL start-of-day/month patterns

```
SELECT CAST(GETDATE() AS date) AS day_start;                        -- midnight today (T-SQL)
...
### ADD_MONTHS / NUMTODSINTERVAL (Oracle)
...
## Differences between dates/timestamps
...
### Date arithmetic & MONTHS_BETWEEN (Oracle)
...
### MAKE_DATE / MAKE_TIMESTAMP (PG)
Construct date/timestamp from components.

```
SELECT MAKE_DATE(2025, 8, 29)                     AS d,            -- (PG)
       MAKE_TIMESTAMP(2025, 8, 29, 14, 30, 0)     AS ts;           -- (PG)
```
...
## “Is in day / month / year?” (index‑friendly)
...
## Time zones (quick reference)

### AT TIME ZONE (PG, T-SQL)
Convert between time zones or attach a zone.

```
-- Convert timestamptz to a zone (display local time)
SELECT NOW() AT TIME ZONE 'Europe/Bucharest' AS bucharest_time;    -- (PG)

-- SQL Server form
SELECT SYSDATETIMEOFFSET() AT TIME ZONE 'E. Europe Standard Time';  -- (T-SQL)
```

### FROM_TZ / CAST (Oracle)
Attach/convert time zones in Oracle.

```
-- Attach zone to a naive timestamp
SELECT FROM_TZ(TIMESTAMP '2025-08-29 14:30:00', 'Europe/Bucharest') AS ts_tz FROM dual;  -- (Oracle)

-- Convert to another zone
SELECT CAST(SYSTIMESTAMP AT TIME ZONE 'Europe/Bucharest' AS TIMESTAMP WITH TIME ZONE) AS ts_buch FROM dual; -- (Oracle)
```
...
```
SELECT DATE_TRUNC('month', NOW())::date AS first_of_month,                 -- (PG)
       (DATE_TRUNC('month', NOW()) + INTERVAL '1 month - 1 day')::date AS last_of_month;  -- (PG)

SELECT TRUNC(SYSDATE, 'MM') AS first_of_month, LAST_DAY(SYSDATE) AS last_of_month FROM dual;  -- (Oracle)

SELECT DATEFROMPARTS(YEAR(GETDATE()), MONTH(GETDATE()), 1) AS first_of_month,  -- (T-SQL)
       EOMONTH(GETDATE()) AS last_of_month;
```
