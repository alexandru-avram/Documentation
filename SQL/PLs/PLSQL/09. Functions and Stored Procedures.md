...
## Local Subprograms
A local subprogram is a procedure or function defined inside another procedure or function.  It's only visible within the enclosing block — like a private helper method.

| Benefit                      | Description                                |
| ---------------------------- | ------------------------------------------ |
| **Encapsulation**            | Logic is hidden and scoped locally         |
| **Modularity**               | Break a complex task into smaller routines |
| **Reusability within block** | Avoid code duplication                     |
| **Readability**              | Keeps related logic grouped together       |

| Block Type      | Can Contain Local Subprograms?     |
| --------------- | ---------------------------------- |
| Anonymous block | ✅ Yes                              |
| Procedure       | ✅ Yes                              |
| Function        | ✅ Yes                              |
| Trigger         | ✅ Yes                              |
| Package         | ❌ No (use regular members instead) |

| Rule                                   | Explanation                                                                  |
| -------------------------------------- | ---------------------------------------------------------------------------- |
| Must be declared **before use**        | Declare local subprograms **before the `BEGIN`** section of the parent block |
| Can **use variables** from outer block | Yes — inherits scope from outer procedure/function                           |
| Cannot be called outside               | Only accessible inside the enclosing block                                   |
| Nested subprograms allowed             | ✅ You can nest local procedures within procedures                            |
...
IS
  -- Local procedure to print employee name
  -- print_employee is only visible inside process_department — nowhere else
  PROCEDURE print_employee(p_name VARCHAR2) IS
  BEGIN
...
BEGIN
  FOR emp IN (SELECT name FROM employees WHERE department_id = p_dept_id) LOOP
    -- Call the local procedure
    print_employee(emp.name);
  END LOOP;
END;
```
