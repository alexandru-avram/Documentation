...
#### REVERSE
Normally, a `FOR` loop counts from a lower number up to a higher number. `REVERSE` flips the direction: it counts backward, from a higher number down to a lower number.

```
FOR counter_variable IN REVERSE upper_bound..lower_bound LOOP
   -- statements
END LOOP;
```

PL/SQL automatically decreases i by 1 in each step.
